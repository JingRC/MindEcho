"""Train LateFusion SqueezeNet for breath INTAKE binary classification.

KEY DIFFERENCE from train_breath_binary_latefusion.py:
- Uses <AP> (Aspirate Pause) annotations as positive labels (NOT breathy phonation)
- Reads anchor_ratio from manifest to center 2.4s windows on breath intakes
- WAV files are longer (2-14s), window is extracted via anchor_ratio
"""
...
def _select_window_bounds(total_length: int, target_length: int, train: bool,
    ...
    if anchor_ratio is not None:
        if train and jitter > 0.0:
            ...
            anchor_ratio = min(1.0, max(0.0, anchor_ratio + jitter_amount))
        start = int(round((total - target) * min(1.0, max(0.0, float(anchor_ratio)))))
        start = max(0, min(total - target, start))
...
def read_audio(path: Path, target_sr: int, *, target_length: int | None = None,
               train: bool = False, anchor_ratio: float | None = None,
    ...
    try:
        ...
        audio = np.asarray(audio, dtype=np.float32).copy()  # detach memmap
    except Exception:
    ...
    if target_length is not None:
        if len(audio) >= target_length:
            if anchor_ratio is not None and not train:
                # Fixed anchor for eval
                start = int(round((len(audio) - target_length) * anchor_ratio))
...
class BreathIntakeLateFusionDataset(Dataset):
    """Dataset that uses anchor_ratio from manifest to center windows on breath intakes."""
    ...
    def __init__(self, rows, *, sample_rate, sample_secs, image_size,
        ...
        self._jitter_window = 0.15  # ±15% jitter around anchor during training

    def __len__(self):
        return len(self.rows)

    def _load_item(self, row, anchor_ratio=None, add_jitter=False):
        audio = read_audio(
            Path(row['wav_path']), self.sample_rate,
            target_length=self.target_length, train=self.train,
            anchor_ratio=anchor_ratio,
            jitter=self._jitter_window if add_jitter else 0.0,
        )
    ...
    def __getitem__(self, index):
        ...
        for attempt in range(max_retries):
            try:
                row = self.rows[index if attempt == 0 else random.randint(0, len(self.rows) - 1)]
                is_breath = int(float(row.get('breath', 0) or 0))
                base_anchor = float(row.get('anchor_ratio', 0.5) or 0.5)

                if not self.train and len(self.eval_anchor_ratios) > 1:
                    mels, specs = [], []
                    for r_offset in self.eval_anchor_ratios:
                        r = max(0.0, min(1.0, base_anchor + r_offset - 0.5))
                        m, s = self._load_item(row, anchor_ratio=r)
                        mels.append(m)
                        specs.append(s)
                    return torch.stack(mels, dim=0), torch.stack(specs, dim=0), is_breath

                if self.train and is_breath:
                    mel, spec = self._load_item(row, anchor_ratio=base_anchor, add_jitter=True)
                elif self.train:
                    mel, spec = self._load_item(row, anchor_ratio=None)
                else:
                    anchor = base_anchor if self.eval_anchor_ratios[0] == 0.5 else self.eval_anchor_ratios[0]
                    mel, spec = self._load_item(row, anchor_ratio=anchor)
                return mel, spec, is_breath
            except Exception:
                if attempt == max_retries - 1:
                    dummy_mel = torch.zeros(3, self.image_size, self.image_size)
                    dummy_spec = torch.zeros(3)
                    return dummy_mel, dummy_spec, 0
                continue
...
def run_training(cfg: TrainConfig) -> Path:
    ...
    train_ds = BreathIntakeLateFusionDataset(train_rows, transform=transform, train=True, **ds_kwargs)
    val_ds = BreathIntakeLateFusionDataset(val_rows, transform=transform, train=False,
                                            eval_anchor_ratios=eval_anchors, **ds_kwargs)
    test_ds = BreathIntakeLateFusionDataset(test_rows, transform=transform, train=False,
                                             eval_anchor_ratios=eval_anchors, **ds_kwargs)
    ...
    report = classification_report(y_true, y_pred,
                                    target_names=['non_breath_intake', 'breath_intake'],
                                    output_dict=True)
    ...
    print(f'F1 (breath_intake): {report["breath_intake"]["f1-score"]:.4f}')
    print(f'Precision (breath_intake): {report["breath_intake"]["precision"]:.4f}')
    print(f'Recall (breath_intake): {report["breath_intake"]["recall"]:.4f}')
    ...
    best_path = cfg.output_dir / 'best_breath_intake_latefusion.pt'
...
def main():
    parser = argparse.ArgumentParser(
        description='Train LateFusion SqueezeNet for breath INTAKE binary classification')
    parser.add_argument('--train-manifest',
                        default=r'd:\-MindEcho-main\ml_dl_models\gtsinger_multitech\dataset\curated\breath_intake_core\train_manifest.csv')
    parser.add_argument('--validation-manifest',
                        default=r'd:\-MindEcho-main\ml_dl_models\gtsinger_multitech\dataset\curated\breath_intake_core\validation_manifest.csv')
    parser.add_argument('--test-manifest',
                        default=r'd:\-MindEcho-main\ml_dl_models\gtsinger_multitech\dataset\curated\breath_intake_core\test_manifest.csv')
    parser.add_argument('--output-dir',
                        default=r'd:\-MindEcho-main\ml_dl_models\gtsinger_multitech\lightweight_training\artifacts\breath_intake_latefusion_v1')
    ...
    parser.add_argument('--sample-secs', type=float, default=1.0)
