...
TARGET = {
    ...
    "wav_path": DATASET_ROOT / "Audio" / "vocadito_21.wav",
    "windows": [
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Probe vocadito_21 residual windows against a chosen repo root.")
    parser.add_argument("--repo-root", required=True, help="Repo root to import runtime code from.")
...
def _hz_to_cents(hz1, ref_hz):
    if hz1 <= 0.0 or ref_hz <= 0.0:
        return None
    try:
        return abs(1200.0 * math.log2(float(hz1) / float(ref_hz)))
    except Exception:
        return None


def _load_reference_rows(track_id):
    path = DATASET_ROOT / "Annotations" / "F0" / f"{track_id}_f0.csv"
    rows = []
...
def _nearest_reference(rows, timeline_time):
    return min(rows, key=lambda item: abs(item[0] - timeline_time))[1]
...
def _summarize_window_errors(window_frames, reference_rows):
    pre_values = []
    post_values = []
    post_better = 0
    pre_better = 0
    ...
    for frame in window_frames:
        ref_hz = _nearest_reference(reference_rows, float(frame.get("timeline_time", 0.0) or 0.0))
        pre_value = _safe_float(frame.get("pre_refine_display", 0.0), 0.0)
        post_value = _safe_float(frame.get("post_refine_display", 0.0), 0.0)
        pre_err = _hz_to_cents(pre_value, ref_hz)
        post_err = _hz_to_cents(post_value, ref_hz)
        if pre_err is not None:
            pre_values.append(pre_err)
        if post_err is not None:
            post_values.append(post_err)
        if abs(pre_value - post_value) > 1e-6:
            changed += 1
        if pre_err is not None and post_err is not None:
            if post_err + 1e-6 < pre_err:
                post_better += 1
            elif pre_err + 1e-6 < post_err:
                pre_better += 1
            else:
                same += 1
    return {
        "pre_median_cents": round(statistics.median(pre_values), 6) if pre_values else None,
        "post_median_cents": round(statistics.median(post_values), 6) if post_values else None,
        "post_better": post_better,
        "pre_better": pre_better,
        "same": same,
        "changed": changed,
    }


def _append_resolved_frame_results(ui, windows, results, window_counters):
    resolve_payload = getattr(ui, "_resolve_technique_analysis_payload", None)
    if not callable(resolve_payload):
        return
    try:
        resolved = dict(resolve_payload() or {})
    except Exception:
        return
    ...
    for index, frame in enumerate(frames):
        timeline_time = _safe_float(getattr(frame, "timeline_time", 0.0), 0.0)
        display_frequency = _safe_float(
            getattr(frame, "display_frequency_hz", 0.0) or getattr(frame, "detected_frequency_hz", 0.0),
            0.0,
        )
        detected_frequency = _safe_float(getattr(frame, "detected_frequency_hz", 0.0), 0.0)
        if not bool(getattr(frame, "has_pitch", False)) or display_frequency <= 0.0:
            continue
        matched_window = None
        ...
        if matched_window is None:
            continue
        window_counters[matched_window["label"]]["resolved_frames_only"] += 1
        results.append(
            {
                "track_id": str(getattr(frame, "track_id", "") or ""),
                "window_label": matched_window["label"],
                "timeline_time": round(timeline_time, 6),
                "payload_index": index,
                "pre_refine_display": round(display_frequency, 6),
                "post_refine_display": round(display_frequency, 6),
                ...
                "payload_raw_frequency": round(0.0, 6),
                "branch_info": {"stage": "resolved_frames_only"},
            }
        )


def _simulate_track(practical, ui, app, track_id, wav_path, windows):
    ...
    reference_rows = _load_reference_rows(track_id)
    results = []
    window_counters = {item["label"]: Counter() for item in windows}
    prev_display = 0.0
    prev_detected = 0.0
    prev_voiced_rms = 0.0
    refine_method = getattr(ui, "_refine_onepass_payload_display_frequency", None)
    ...
    for index, pitch_data in enumerate(payloads):
        ...
        if callable(refine_method):
            ...
            branch_info["stage"] = "no_refine_method"
            actual_refined = float(current_frequency)
    ...
    if not results:
        _append_resolved_frame_results(ui, windows, results, window_counters)

    summary_windows = []
    for window in windows:
        label = window["label"]
        counts = dict(window_counters[label])
        frames = [frame for frame in results if frame.get("window_label") == label]
        summary_windows.append(
            {
                ...
                "error_summary": _summarize_window_errors(frames, reference_rows),
            }
        )
    return {
        "repo_root": str(Path(sys.path[0]).resolve()),
        "track_id": track_id,
        "wav_path": str(wav_path),
        "has_refine_method": bool(callable(refine_method)),
...
def main() -> int:
    ...
    ui = None
    try:
        practical, app, ui = _load_runtime(repo_root)
        payload = {
            "targets": [
                _simulate_track(
                    practical,
        ...
        if practical is not None and app is not None and ui is not None:
            try:
    ...
    summary_text = ", ".join(
        f"{window['label']}={window['branch_counts']} err={window['error_summary']}"
        for window in payload["targets"][0].get("window_summaries", [])
    )
    print(f"[probe] {TARGET['track_id']} refine={payload['targets'][0]['has_refine_method']} {summary_text}", flush=True)
    return 0
