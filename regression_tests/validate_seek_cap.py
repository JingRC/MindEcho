...
def main():
    ...
    # Configure a manageable window and ensure record-active like state
    ui.visualizer.time_offset = 0.0
    ui.visualizer.time_window = 20.0
    ui.visualizer.is_recording_active = True
    ...
    assert_true(len(pts_pre) > 50, 'Precondition: not enough visible points after feed')
    ...
    # After seek, check that there are points with x <= 10 and also some line/points visible immediately
    pts_after_seek = extract_visible_points(ui)
    assert_true(any(x <= 10.0 + 1e-3 for x, _ in pts_after_seek), 'Seek: <=10s points should remain visible')
    assert_true(len(pts_after_seek) > 0, 'Seek: should render immediately, not empty')

    # 3) Continue feeding from 10s onward for 3s; we should see new points immediately (no long snowflake-only delay)
    feed_points(ui, 10.0, total_sec=3.0)
    ui.visualizer.update_display()
    wait_qt(200)
    pts_after_more = extract_visible_points(ui)
    # There should be points > 10.3 quickly
    assert_true(any(x > 10.3 for x, _ in pts_after_more), 'Post-seek: new segments/points should appear quickly')
    ...
    assert_true(all(x <= 8.0001 for x, _ in pts_cap), 'Cap: points beyond cap should not reappear')

    print('OK: validate_seek_cap passed')
